...
class NSRBuffer:
    ...
    def __init__(
        ...
        self.ter_buf = np.zeros([max_size, 1], dtype=np.float32)
        self.ptr, self.size, self.max_size, self.batch_size = 0, 0, max_size, batch_size
        self.n_steps, self.n_steps_sarsd = n_steps, deque(maxlen=n_steps)
        self.gamma = gamma
        self._device_ = device
    ...
    def store_sarsd(
        ...
        self.n_steps_sarsd.append(sarsd)
        if len(self.n_steps_sarsd) < self.n_steps:
            # print('len(self.n_steps_sarsd) < self.n_steps')
            return ()

        # print('len(self.n_steps_sarsd) >= self.n_steps')
        o, a = self.n_steps_sarsd[0][:2]
        r, o_next, d = self._get_n_steps_info(self.n_steps_sarsd, self.gamma)
        ...
        return self.n_steps_sarsd[0]

    def sample_batch(self, batch_size=32, device='cpu') -> Dict[str, np.ndarray]:
        # print(f'bs={batch_size} | size={self.size}')
    ...
    def _get_n_steps_info(
        self,
        n_steps_sarsd: Deque,
        gamma: float) -> Tuple[float, np.ndarray, bool]:
        reward, observation_next, terminal = self.n_steps_sarsd[-1][-3:]
        for sards in reversed(list(n_steps_sarsd)[:-1]):
            r, o_next, d = sards[-3:]
...
class NSPERBuffer(NSRBuffer):
    ...
    def __init__(
        ...
        seed = 0, device = 'cpu'):
        assert alpha >= 0
        super(NSPERBuffer, self).__init__(
                obs_dim,
                max_size,
                n_steps=n_steps,
