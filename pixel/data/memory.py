...
class ReplayMemory():
  ...
  def __init__(self, capacity):
    ...
    self.history = 4 # args.history_length
    self.discount = 0.99 #args.discount
    self.n = 20 #args.multi_step
    self.priority_weight = 0.6 #args.priority_weight  # Initial importance sampling weight β, annealed to 1 over course of training
    self.priority_exponent = 0.4 #args.priority_exponent
  ...
  def append_sard(self, state, action, reward, terminal):
    ...
    state = (state[-1]*255).astype(np.uint8)
  ...
  def sample_batch(self, batch_size):
    ...
    batch = dict(tree_idxs=tree_idxs,
                 observations=states,
                 actions=actions,
                 returns=returns,
