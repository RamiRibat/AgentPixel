...
class RainbowLearner(MFRL):
    ...
    def learn(self):
        ...
        # RainbowLT = trange(1, LT+1, n_envs, desc=alg)
        RainbowLT = tqdm(total=LT, desc=alg, position=0)
        # RainbowLT.refresh()
        # RainbowLT.set_postfix({'T': f'{t}/{LT}', 'L': np.mean(ZList), 'V': np.mean(VZ)})
        # observation, info = self.learn_envs.reset()
        # mask = np.ones([n_envs], dtype=bool)
        logs, ZList, LList, JQList = dict(), [0], [0], []
        # update_counter, target_counter, log_counter = 0, 0, 0
        # EPS = []
        ...
        with RainbowLT:
            ...
            T, I = 0, 0
            while True:
                observation, mask, Z, L, Traj_new = self.interact_vec(observation, mask, Z, L, T, Traj)
                ...
                if (I%Vf==0):
                    # print(f'i={I} | eval')
                    if (I) >= Vf:
                        print(f'buffer.observation[{I-Vf}:{I}]', self.buffer_n.obs_buf[I-Vf:I])
                    ...
                    RainbowLT.set_postfix({'T': f'{T}/{LT}', 'Z': np.mean(ZList), 'V': np.mean(VZ)})
...
def main(exp_prefix, config, seed, device, wb):
    ...
    group_name = f"{env_name}-{alg_name}-X{num_envs}-2" # H < -2.7
    exp_prefix = f"seed:{seed}"
