...
def _init_weights(l):
    ...
    if isinstance(l, NoisyLinear):
        nn.init.trunc_normal_(l.weight_mu, mean=0.0, std=1e-4)
        nn.init.trunc_normal_(l.weight_sigma, mean=0.0, std=1e-4)
        nn.init.constant_(l.bias_mu, 0.0)
        nn.init.constant_(l.bias_sigma, 0.0)
...
class NoisyLinear(nn.Module):
    ...
    # def forward_(self, x: T.Tensor) -> T.Tensor:
    ...
    def reset_parametrs(self):
        ...
        self.bias_sigma.data.fill_(self.std_init / math.sqrt(self.out_features)) # Error: in_features
...
class NoisyNetwork(nn.Module):
    ...
    def __init__(self, in_dim: int, out_dim: int, configs: Dict):
        ...
        # self.h = NoisyLinear(in_features=in_dim, out_features=configs['arch'][1], std_init=configs['std'])
        # self.z = NoisyLinear(in_features=configs['arch'][1], out_features=out_dim, std_init=configs['std'])

        # self.apply(_init_weights)

    def forward(self, x: T.Tensor) -> T.Tensor:
        return self.net(x)
        # return self.z(F.relu(self.h(x)))
